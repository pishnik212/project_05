...
def upload_to(instance, filename):
    return os.path.join('uploads', str(datetime.date.today().year), filename)
...
class Filesnew(models.Model):
    ...
    Filesnew = models.FileField(upload_to=upload_to, blank=True, null=True)  # Новое поле для файла
